...
'''     
    def __str__(self):
        # Use vars(self) to get a dictionary of the current attributes of the object
        attributes_dict = vars(self)
        return attributes_dict
        #return ""

   #(Png, Png.PhysUnit, Png.BlendOpValues,Png.CompressionMethods,Png.DisposeOpValues,Png.ColorType,Png.Rgb,Png.Chunk,Png.BkgdIndexed,Png.Point,Png.BkgdGreyscale,Png.ChrmChunk,Png.IhdrChunk,Png.PlteChunk,Png.SrgbChunk,Png.Intent,Png.CompressedTextChunk,Png.FrameDataChunk,Png.BkgdTruecolor,Png.GamaChunk,Png.BkgdChunk,Png.PhysChunk,Png.FrameControlChunk,Png.InternationalTextChunk,Png.TextChunk,Png.AnimationControlChunk,Png.TimeChunk)):
        # Optionally, can recursively convert contained objects
        for attr in attributes_dict.keys():
            if isinstance(attributes_dict[attr], list):
                elements = []
                for ele in attributes_dict[attr]:
                    elements.append(str(ele)+"\n")
                attributes_dict[attr] = elements 
            elif isinstance(attributes_dict[attr], Png.IhdrChunk):
                attributes_dict[attr] = str(attributes_dict[attr])
            if isinstance(attributes_dict[attr], (Png.IhdrChunk)):
                print("y", attr)
